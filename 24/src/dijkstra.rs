...
impl<const N: usize, V> StateMap<Ve<N>, V> for Grid<N, Vec<Option<(usize, V)>>> {
	fn insert_if_better(&mut self, key: &Ve<N>, value: (usize, V)) -> bool {
	...
	fn is_worse(&self, key: &Ve<N>, value: usize) -> bool {
		match &self[*key] {
			None => false,
			Some(v) => v.0 < value,
		}
	...
	fn get(&self, key: &Ve<N>) -> Option<&(usize, V)> {
		if let Some(Some(v)) = self.get(*key) {
			Some(v)
		} else {
			None
		}
